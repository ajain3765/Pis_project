<?php
	...
	{	
		...
		$pass = mysqli_real_escape_string($con,$pass);
         $result = mysqli_query($con,"SELECT email FROM admin1 WHERE email = '$email' and password = '$pass'") or die('Error');
        ...
        if($count>=1)
        {
            ...
            $_SESSION["name"] = 'Admin1';
            $_SESSION["key"] ='admin1';
            $_SESSION["email"] = $email;
            header("location:welcome_admin.php?q=0");
        }
		else{
		$result = mysqli_query($con,"SELECT email FROM admin WHERE email = '$email' and password = '$pass'") or die('Error');
        $count=mysqli_num_rows($result);
        if($count>=1)
        {
            session_start();
