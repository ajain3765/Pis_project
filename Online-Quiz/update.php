<?php
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['femail'] && $_SESSION['key']=='admin1') 
    {
      $demail=@$_GET['femail'];
      $result = mysqli_query($con,"DELETE FROM admin WHERE email='$demail' ") or die('Error');
      header("location:welcome_admin.php?q=2");
    }
  }
  if(isset($_SESSION['key']))
  {
  ...
  if(isset($_SESSION['key']))
  {
    ...
    if(@$_GET['q']== 'addquiz' && $_SESSION['key']=='admin') 
    {
      ...
      $wrong = $_POST['wrong'];
	   $good = $_POST['right'];
      $id=uniqid();
      $q3=mysqli_query($con,"INSERT INTO quiz VALUES  ('$id','$title' , '$good', '$wrong','$total', NOW())");
	  $q4=mysqli_query($con,"INSERT INTO quiz_admin VALUES ('$id' , '$quizadmin')") or die('Error');
  ...
  {
	  //var_dump($_SESSION['key']);
    if(@$_GET['q']== 'add_teacher' && $_SESSION['key']=='admin1') 
    {
      $title = $_POST['name'];
      $title= ucwords(strtolower($title));
      $email = $_POST['email'];
      $id=uniqid();
      $q3=mysqli_query($con,"INSERT INTO admin VALUES  ('$id' , '$title' , '$email' , '$id')");
      header("location:welcome_admin.php?q=3");
  ...
  if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) 
  {
    ...
    if($ans == $ansid)
    {
      ...
      $r++;
      $s=$s+$good;
      $q=mysqli_query($con,"UPDATE `history` SET `score`=$s,`level`=$sn,`good`=$r, date= NOW()  WHERE  email = '$email' AND eid = '$eid'")or die('Error124');
    } 
    else
    {
